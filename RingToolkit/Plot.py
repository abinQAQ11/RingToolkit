...
def plot_tunes(tunes, obj, var: str = "emittance", var_index: int = 0):
    ...
    #         # if v[0] < 8 and v[1] < 12 and v[2] < 0.025:
    #         if v[2] < 0.04:
    #             obj_.append(v[:] + [i + 1])
    ...
    x0 = np.array([7.1626327108374515])
    y0 = np.array([2.6249906811790904])
    ...
    # ax.plot(2.5 * np.linspace(y_min, y_max, 1000), np.linspace(y_min, y_max, 1000),
    #         color='red', alpha=0.5, zorder=4, label='x = 2y')
...
def plot_pareto(data: List):
    ...
    ax.set_xlabel('emittance [nm]', fontsize=25)
